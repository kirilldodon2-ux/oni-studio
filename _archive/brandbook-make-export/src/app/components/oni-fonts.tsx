...
interface OniFontsProps {
  sectionRef: { current: HTMLElement | null }
}

export function OniFonts({ sectionRef }: OniFontsProps) {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative bg-[#F7F7F7] overflow-hidden"
      ...
      <div className="absolute top-0 bottom-0 left-1/2 pointer-events-none">
        ...
        >
          <svg className="h-full" width="3" fill="none">
            <line x1="1.5" y1="0" x2="1.5" y2="100%" stroke="#B2B2B2" strokeWidth="2" strokeDasharray="8 8" />
          </svg>
        </motion.div>
      </div>
        ...
        <div className="w-1/2 flex flex-col justify-center px-[6%] gap-8">
          {TYPEFACES.map((face, i) => (
            ...
            >
              ...
              <p
                className="text-[#B2B2B2] mb-3 tracking-widest"
                style={{
                  fontFamily: 'var(--font-mono)',
                  fontSize: '10px',
                  fontWeight: 300,
